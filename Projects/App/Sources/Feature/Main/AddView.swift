...
struct AddView: View {
    ...
    let statisticsStore: StoreOf<StatisticsFeature> = Store(
        initialState: StatisticsFeature.State(), reducer: { StatisticsFeature() })
    let completionStore: StoreOf<CompletionFeature> = Store(
        initialState: CompletionFeature.State(), reducer: { CompletionFeature() })
                        ...
                        VStack(alignment: .center) {
                                ...
                                Text("저장")
                                    .onTapGesture {
                                        if viewStore.mode == .editing {
                                            viewStore.send(
                                                .updateHabit(
                                                    name: viewStore.habitTitle,
                                                    iter: viewStore.iter,
                                                    habit: viewStore.selectedHabit ?? Habit()))
                                        } else {
                                            viewStore.send(
                                                .addHabit(
                                                    name: viewStore.habitTitle, iter: viewStore.iter
                                                ))
                                        }
                                        viewStore.send(.setViewMode)
                                        viewStore.send(.setHabitTitle(""))

                                        statisticsStore.send(
                                            .setnumOfToDo(
                                                add: true, numOfIter: viewStore.iter.count))
                                        statisticsStore.send(.getnumOfToDo)

                                        completionStore.send(
                                            .updateAllDoneContinuity(
                                                .add, isTodayHabit(viewStore.iter) ? true : false))
                                    }
                            }
                            ...
                            TextField(
                                ...
                                text: viewStore.binding(
                                    get: \.habitTitle, send: { .setHabitTitle($0) })
                            )
                            ...
                            HStack {
                                ForEach(1..<8) {
                                    WeekButton(
                                        weekOfDay: $0,
                                        iter: viewStore.binding(
                                            get: \.iter,
                                            send: HabitFeature.Action.setIter
                                        ),
                                        OnOff: viewStore.selectedHabit?.weekIter.contains($0)
                                            ?? false ? true : false)
                                }
                            }
